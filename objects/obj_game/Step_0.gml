...
if (!moving) {
    ...
    if (move_cooldown <= 0) {
        ...
        if (keyboard_check_pressed(ord("W"))) {
            ...
            if (new_y >= 0 && new_y < array_length(global.map)
             && new_x >= 0 && new_x < array_length(global.map[new_y])
             && global.map[new_y][new_x] == 0)
            {
    ...
    if (move_progress >= 1.0) {
        ...
        var ty = move_target_y;
        player_real_x = lerp(sx, tx, move_progress);
        player_real_y = lerp(sy, ty, move_progress);
    }
}
